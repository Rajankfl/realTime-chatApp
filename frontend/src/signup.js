...
function Register() {
  ...
  const postData = async (e) => {
    e.preventDefault();
    const res = await fetch(`${apiConfig.baseURL}/register`, {
      method: "POST",
      headers: {
    ...
    if (res.status === 422 || !data) {
      ...
      alertify
        ...
        .set({ transition: "flipy", message: "Fill All The Data" })
        .show();
    ...
    if (res.status === 201) {
      ...
      alertify
        .dialog("alert")
        .setHeader("<em> Chat App </em> ")
        .set({ transition: "pulse", message: "You Are Registered SucessFully" })
        .show();
      history.push("/login");
    }
    if (res.status === 400) {
      ...
      alertify
        .dialog("alert")
        .setHeader("<em> Chat App </em> ")
        .set({ transition: "flipy", message: "Invalid Data Entered" })
        .show();
    }
    if (res.status === 403) {
      alertify
        ...
        .setting({
          ...
          message:
            "This dialog is : " + (closable ? " " : " not ") + "closable.",
          onok: function () {
            alertify.error("Try Differ Email");
          },
        })
        .show();
      alertify
